...
def generate_education_and_scientific_structural_divisions_md_file(division_structure, path: Path):
    path = Path(path, EDUCATIONAL_AND_SCIENTIFIC_STRUCTURAL_DIVISIONS_DIRECTORY_NAME)
    path.mkdir(parents=True, exist_ok=True)

    md_file = MdUtils(file_name=str(Path(
        path,
    ...
    md_file.new_header(2, "Полное название")
    add_code_sources_to_md_file(md_file, division_structure.full_name)
    md_file.new_header(2, "Короткое название")
...
def generate_branches_md_file(branch_structure, path: Path):
    ...
    md_file = MdUtils(file_name=str(Path(
        path,
        f"{from_tex_to_text(branch_structure.short_name)}.md")))
    ...
    if len(branch_structure.departments) > 0:
        md_file.new_header(2, "Структура")
...
def generate_laguage_specific_structure(rtu_mirea_structure, lang: str):
    ...
    for division in rtu_mirea_structure_lang.educational_and_scientific_structural_divisions:
...
def main():
    ...
    for lang, _ in rtu_mirea_structure.__dict__.items():
    ...
    with open("ru_RU/README.md", "r") as rm_ru_file:
        ...
        rm_file.new_paragraph(rm_file.new_reference_link("en_US/README.md", "English names"))
        rm_file.new_paragraph(rm_ru_file.read()
                              .replace(EDUCATIONAL_AND_SCIENTIFIC_STRUCTURAL_DIVISIONS_DIRECTORY_NAME,
                                       str(Path("ru_RU",
                                                EDUCATIONAL_AND_SCIENTIFIC_STRUCTURAL_DIVISIONS_DIRECTORY_NAME)))
                              .replace(BRANCHES_DIRECTORY_NAME, str(Path("ru_RU", BRANCHES_DIRECTORY_NAME))))
